<script lang="ts">
    ...
    let positions = intRange(0, Math.max(0, shift.needed - 1, shift.users.length - 1));

</script>

<div class="flex flex-col border-2 border-gray-500 dark:border-neutral-800 border-opacity-25 rounded shadow">
    <div class="flex flex-row justify-between items-center px-4 py-2">
        <div class="flex flex-col">
            <div class="text-lg font-semibold text-neutral-800 dark:text-neutral-200">
                {shift.name}
                <div class="text-md text-neutral-700 dark:text-neutral-300 text-sm">
                    {datetimeFormatter.format(new Date(shift.from_time.replace(" ", "T")))}
                    - {datetimeFormatter.format(new Date(shift.to_time.replace(" ", "T")))}
                    {#if shift.needed < shift.users.length} - &Uuml;berbesetzt ({shift.users.length - shift.needed})
                    {/if}
                </div>
            </div>
            <div class="text-sm">
                {#each positions as position}
                    <span class="text-neutral-600 dark:text-neutral-400">{#if shift.users[position]}{shift.users[position].firstname} {shift.users[position].lastname}{:else}unbesezt{/if}</span>{#if position < shift.needed - 1}<span class="text-neutral-200 dark:text-neutral-400">, </span>{/if}
                {/each}
            </div>
        </div>
        <div class="flex flex-row gap-2 items-end">
            {#if user.permission.includes("joinShifts")}
                <span>
                    {#if shift.users.find(u => u.id === user.id)}
                        <button class="p-1 px-2 bg-tertiary text-white rounded hover:bg-tertiary_highlight flex gap-2 items-center"
                    ...
                    {:else}
                        <button class="p-1 px-2 bg-primary text-white rounded hover:bg-primary_highlight flex gap-2 items-center"
            ...
            {#if user.permission.includes("editShifts")}
                <button class="p-1 px-2 bg-primary text-white rounded hover:bg-primary_highlight block h-full"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="23" height="23">
                        ...
                        <line x1="164" y1="92" x2="68" y2="188" fill="none" stroke="currentColor" stroke-linecap="round"
                              stroke-linejoin="round" stroke-width="16"/>
                        <line x1="95.49" y1="215.49" x2="40.51" y2="160.51" fill="none" stroke="currentColor"
                              stroke-linecap="round" stroke-linejoin="round" stroke-width="16"/>
                    </svg>
                </button>
                <button class="p-1 px-2 bg-primary text-white rounded hover:bg-primary_highlight block h-full"
